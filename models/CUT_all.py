...
sys.stdout = open("{}_{}.log".format("CUTall",file_name), 'w')
    ...
    #     val = model._vals
        ...
        if status == GRB.OPTIMAL:
            ...
            for center in centerList:
                ...
                value = center[1]
                #only add cuts for center that has great Xcc value and  when no cut is added
                if value < ((1/VNum) - 0.001) and (not hasAdditionalCuts):
                   break
...
with open("{}_{}.pk".format("CUTall",file_name),"wb") as f:
    pk.dump(CUTALLSol,f)
